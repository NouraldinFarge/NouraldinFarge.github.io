...
Each publication is produced from the version-controlled [portfolio source](https://github.com/NouraldinFarge/portfolio-source) after its rendered-HTML, static-export, desktop/mobile Chrome and Axe, lint, and production-build checks pass. The exporter copies only referenced assets, rejects executable bundles and local development paths, and verifies that the deployed résumé matches the current ATS-tested PDF. This repository also validates the generated artifact independently in GitHub Actions. Generated HTML and CSS should not be edited by hand.
