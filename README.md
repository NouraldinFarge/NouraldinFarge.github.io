...
The site also has a deliberately separate [Active source, backed by evidence](https://nouraldinfarge.github.io/#active-source) section. Media Scout Downloader v3.7.13 and Reader v0.1.0-alpha.4 have image-backed engineering evidence while remaining explicitly labeled as a Chrome MV3 prerelease and a public-source Windows alpha. SiteWipe stays compact and release-gated. None is presented as a shipped product, supported download, or extension-store release.
...
Each publication is produced from a clean, version-controlled [portfolio source](https://github.com/NouraldinFarge/portfolio-source) revision after its rendered-HTML, static-export, desktop/mobile Chrome and Axe, lint, and production-build checks pass. The exporter rejects tracked or untracked non-ignored source changes, copies only referenced assets, rejects executable bundles and local development paths, verifies that the deployed résumé matches the current ATS-tested PDF, and writes the exact source revision to [`portfolio-build.json`](portfolio-build.json). This repository validates the generated artifact independently on every pull request, including the exact hashes and preserved notices for the two reused MIT-licensed project screenshots. After changes reach `main`, a second least-privilege job deploys only the verified artifact through SHA-pinned official GitHub Pages actions. Generated HTML and CSS should not be edited by hand.
